...
static bool virtio_vsock_transport_registered;
...
static int virtio_vsock_probe(struct virtio_device *vdev)
{
	...
	vsock->vdev = vdev;
	if (!virtio_vsock_transport_registered) {
		ret = vsock_core_register(&virtio_transport.transport,
			VSOCK_TRANSPORT_F_G2H);
		if (ret)
			goto out;
		virtio_vsock_transport_registered = true;
	}
...
out:
	if ((ret != 0) && virtio_vsock_transport_registered) {
		vsock_core_unregister(&virtio_transport.transport);
		virtio_vsock_transport_registered = false;
	}
	kfree(vsock);
	mutex_unlock(&the_virtio_vsock_mutex);
	return ret;
