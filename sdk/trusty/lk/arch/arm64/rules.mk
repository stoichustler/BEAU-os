...
WITH_TRUSTY_APP_CRASH ?= true

ifeq (true,$(call TOBOOL,$(WITH_TRUSTY_APP_CRASH)))
GLOBAL_DEFINES += WITH_TRUSTY_APP_CRASH=1

MODULE_DEPS += \
	trusty/kernel/lib/trusty \

else
GLOBAL_DEFINES += WITH_TRUSTY_APP_CRASH=0
...
ARM64_LINKER_TEMPLATE ?= $(LOCAL_DIR)/system-onesegment.ld
ARM64_LINKER_OUTPUT ?= system-onesegment.ld

GENERATED += \
	$(BUILDDIR)/$(ARM64_LINKER_OUTPUT)

# rules for generating the linker script
$(BUILDDIR)/$(ARM64_LINKER_OUTPUT): $(ARM64_LINKER_TEMPLATE) $(wildcard arch/*.ld) linkerscript.phony
	@echo generating $@
	@$(MKDIR)
...
LINKER_SCRIPT += $(BUILDDIR)/$(ARM64_LINKER_OUTPUT)
