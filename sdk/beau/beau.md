...
## Virtio-proxy Fs, Rng, And Blk Validation

The QEMU virtio-proxy topology keeps virtio-fs, virtio-rng, and virtio-blk
...
- `arch/arm64/platform/qemu/platform.dts` advertises three proxy endpoints:
  ...
  and blk at `0x0a000600` with `beau,device-id = <2>; beau,throughput = "high";`.
...
- VM1 Linux enables `CONFIG_BEAU_VIRTIOFS_BACKEND=y`,
  `CONFIG_BEAU_VIRTIORNG_BACKEND=y`, and `CONFIG_BEAU_VIRTIOBLK_BACKEND=y`.
- VM2 Linux exposes all three virtio-mmio frontend nodes in its DTB.
- VM2 Linux enables `CONFIG_HW_RANDOM_VIRTIO=y` and `CONFIG_VIRTIO_BLK=y`.
