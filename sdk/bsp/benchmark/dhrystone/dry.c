...
#ifdef BEAU_DHRYSTONE
#include "dhrystone_port.h"

#define CLOCK_TYPE "BEAU cpu_ticks()"
#define HZ dhrystone_port_ticks_per_second()
#define Too_Small_Time ((long)(2UL * HZ))
#define Start_Timer() Begin_Time = (long)dhrystone_port_ticks()
#define Stop_Timer() End_Time = (long)dhrystone_port_ticks()
...
#ifdef BEAU_DHRYSTONE
static Rec_Type beau_records[2];
#endif
  ...
  /* Main and Proc_0 in the Ada version             */
{
...
#ifdef BEAU_DHRYSTONE
  uint64_t              Accumulated_Runs = 0UL;
#endif

  /* Arguments */
#ifdef BEAU_DHRYSTONE
  if ((result == 0) || !dhrystone_port_acquire())
  {
     return -EBUSY;
  }
  memset (result, 0, sizeof (*result));
  Number_Of_Runs = initial_runs == 0 ? NUMBER_OF_RUNS : (int)initial_runs;
#else
  if (argc > 2)
  {
...
#ifdef BEAU_DHRYSTONE
  memset (beau_records, 0, sizeof (beau_records));
  memset (Arr_1_Glob, 0, sizeof (Arr_1_Glob));
  memset (Arr_2_Glob, 0, sizeof (Arr_2_Glob));
  Next_Ptr_Glob = &beau_records[0];
  Ptr_Glob = &beau_records[1];
#else
  ...
  while (!Done) {
...
#ifdef BEAU_DHRYSTONE
    Accumulated_Runs += (uint32_t)Number_Of_Runs;
#endif
    ...
    if (User_Time < Too_Small_Time)
    {
      ...
      printf ("too few\n");
#else
      if ((uint32_t)Number_Of_Runs >
          (CONFIG_DHRYSTONE_MAX_INITIAL_RUNS / 10U))
      {
        dhrystone_port_release();
        return -EINVAL;
      }
#endif
  ...
  fprintf (stderr, "\n");
    ...
    printf ("\n");
#else
    result->runs = (uint32_t)Number_Of_Runs;
    result->elapsed_ticks = (uint64_t)User_Time;
    result->valid = (Int_Glob == 5) && (Bool_Glob == true) &&
                    (Ch_1_Glob == 'A') && (Ch_2_Glob == 'B') &&
                    (Arr_1_Glob[8] == 7) &&
                    (Arr_2_Glob[8][7] == (int)(Accumulated_Runs + 10UL)) &&
                    ...
                    (Int_3_Loc == 7) && (Enum_Loc == Ident_2);
    dhrystone_port_release();
    return result->valid ? 0 : -EINVAL;
#endif
