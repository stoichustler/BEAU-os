...
void panic_dump_context(void)
{
	...
	LOG_ERR("ctx: pcpu:%hu    active:0x%lx   mpidr:0x%lx  daif:0x%lx",
		pcpu_id, get_active_pcpu_bitmap(), read_mpidr_el1(), read_daif());
	LOG_ERR("el2:  elr:0x%08lx  spsr:0x%08lx   esr:0x%08lx far:0x%08lx hpfar:0x%08lx",
		read_elr_el2(), read_spsr_el2(), esr, read_far_el2(),
		read_hpfar_el2());
	LOG_ERR("virt: hcr:0x%08lx vttbr:0x%08lx sctlr:0x%08lx",
		read_hcr_el2(), read_vttbr_el2(), read_sctlr_el2());

	if ((vcpu != NULL) && (vcpu->vm != NULL)) {
		LOG_ERR("vcpu: vm:%hu vcpu:%hu state:%u pcpu:%hu pending:0x%lx",
			vcpu->vm->vm_id, vcpu->vcpu_id, vcpu_get_state(vcpu),
			pcpuid_from_vcpu(vcpu), vcpu->pending_req);
	} else {
		LOG_ERR("vcpu: none");
	}
	arm64_coredump_log(&context, pcpu_id);
	if (pcpu_id < MAX_PCPU_NUM) {
